  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        let res = await fetch(`/api/news?language=english&country=${countryName}&category=world&from=9&to=18`);
        ...
        if (!fetchedArticles.length) {
          ...
          res = await fetch(`/api/news?language=english&category=top&category=world&from=9&to=18`);
          json = await res.json();
